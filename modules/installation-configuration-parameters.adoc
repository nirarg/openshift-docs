...
When deploying {product-title} cluster with remote workers running in AWS Outposts, some customization steps are required in order to provide the specific details of the Outposts environment.
Specific requirements and instructions for AWS Outposts.

[NOTE]
====
The AWS Outposts environments has the following limitations which require manual modification of the

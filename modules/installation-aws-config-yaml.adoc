...
ifdef::aws-outposts[]
When running remote workers in AWS Outposts, the following fields must be modified:
* The Compute Platform section must be modified with supported instance type and rootVolume type.
* List of all subnets in the VPC, 2 subnets (private and public) in availability zone which are not in the Outposts.
endif::aws-outposts[]

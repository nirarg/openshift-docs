...
ifdef::aws-outposts[]
In {product-title} {product-version}, AWS Subnets can't be deployed automatically on AWS Outposts by the installer, therefore you have to configure the VPC manually.
endif::aws-outposts[]
...
ifdef::aws-outposts[]
* In the availability zone in which the AWS Outposts is installed, you must create a public and private subnets inside the Outpost, in addition to the ones mentioned above. This is achieved by indicating the Outposts ARN during the subnet creation.
+
The Outposts subnets you've created must not be used in the `platform` section of the `install-config.yaml` file. Record the private subnet record ID for the worker machines machineset creation.
endif::aws-outposts[]
...
ifdef::aws-outposts[]
+
Special instructions for the Outposts subnets: in order to prevent these subnets from being used by the AWS LoadBalancer, they must include the `kubernetes.io/cluster/.*-outposts: outposts` special tag.
endif::aws-outposts[]
...
ifndef::aws-china,aws-secret,aws-outposts[]
* `ec2.<region>.amazonaws.com`
* `elasticloadbalancing.<region>.amazonaws.com`
* `s3.<region>.amazonaws.com`
endif::aws-china,aws-secret,aws-outposts[]
